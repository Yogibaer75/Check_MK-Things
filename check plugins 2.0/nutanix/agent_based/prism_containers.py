...
from .agent_based_api.v1 import get_value_store, register, Service, Result, State, Metric
...
def check_prism_container(item: str, params: Mapping[str, Any], section: Section) -> CheckResult:
    ...
    if data.get("fingerPrintOnWrite") == "on" and dedupratio != -1:
        dedupratio = dedupratio/1000000
        summary = f"Dedup Ratio: {dedupratio:.2f}"
        yield Metric("dedup_ratio", dedupratio)
