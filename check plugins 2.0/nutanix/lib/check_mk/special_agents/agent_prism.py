...
def write_title(section: str, separator: int) -> None:
...
def output_pools(requester: Requester) -> None:
    write_title("storage", 0)
    obj = requester.get("storage_pools")
    print(obj)
...
def agent_prism_main(args: Args) -> None:
    ...
    LOGGING.info("fetch and write storage pool info..")
    output_pools(requester)
