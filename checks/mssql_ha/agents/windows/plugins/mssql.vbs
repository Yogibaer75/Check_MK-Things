...
sections.add "highavailablity2", "<<<mssql_ha2>>>"
...
For Each instance_id In instances.Keys: Do ' Continue trick
    ...
    If section_ha = 1 Then
        ...
        addOutput(sections("highavailablity2"))

        RS.Open " SELECT ars.role ," &_
            " drs.database_id ," &_
            " drs.replica_id ," &_
            " drs.last_commit_time" &_
        " INTO   #tmpdbr_database_replica_states_primary_LCT" &_
        " FROM   master.sys.dm_hadr_database_replica_states AS drs" &_
            " LEFT JOIN master.sys.dm_hadr_availability_replica_states ars ON drs.replica_id = ars.replica_id" &_
        " WHERE  ars.role = 1;", CONN
        RS.Open " SELECT   AR.replica_server_name AS [AvailabilityReplicaServerName] ," &_
                " dbcs.database_name AS [AvailabilityDatabaseName] ," &_
                " AG.name AS [AvailabilityGroupName] ," &_
                " ISNULL(dbr.database_id, 0) AS [DatabaseId] ," &_
                " CASE dbcs.is_failover_ready" &_
                    " WHEN 1 THEN 0" &_
                    " ELSE" &_
                        " ISNULL(" &_
                            " DATEDIFF(" &_
                                " ss ," &_
                                " dbr.last_commit_time," &_
                                " dbrp.last_commit_time" &_
                            " ) ," &_
                        " 0" &_
                    " )" &_
                " END AS [EstimatedDataLoss] ," &_
                " ISNULL(   CASE dbr.redo_rate" &_
                    " WHEN 0 THEN -1" &_
                        " ELSE CAST(dbr.redo_queue_size AS FLOAT) / dbr.redo_rate" &_
                        " END ," &_
                        " -1" &_
                    " ) AS [EstimatedRecoveryTime] ," &_
                " ISNULL(dbr.filestream_send_rate, -1) AS [FileStreamSendRate] ," &_
                " ISNULL(dbcs.is_failover_ready, 0) AS [IsFailoverReady] ," &_
                " ISNULL(dbcs.is_database_joined, 0) AS [IsJoined] ," &_
                " arstates.is_local AS [IsLocal] ," &_
                " ISNULL(dbr.is_suspended, 0) AS [IsSuspended] ," &_
                " ISNULL(dbr.last_commit_time, 0) AS [LastCommitTime] ," &_
                " ISNULL(dbr.last_hardened_time, 0) AS [LastHardenedTime] ," &_
                " ISNULL(dbr.last_received_time, 0) AS [LastReceivedTime] ," &_
                " ISNULL(dbr.last_redone_time, 0) AS [LastRedoneTime] ," &_
                " ISNULL(dbr.last_sent_time, 0) AS [LastSentTime] ," &_
                " ISNULL(dbr.log_send_queue_size, -1) AS [LogSendQueueSize] ," &_
                " ISNULL(dbr.log_send_rate, -1) AS [LogSendRate] ," &_
                " ISNULL(dbr.redo_queue_size, -1) AS [RedoQueueSize] ," &_
                " ISNULL(dbr.redo_rate, -1) AS [RedoRate] ," &_
                " ISNULL(AR.availability_mode, 2) AS [ReplicaAvailabilityMode] ," &_
                " arstates.role_desc AS [ReplicaRole] ," &_
                " dbr.suspend_reason_desc AS [SuspendReason] ," &_
                " ISNULL(" &_
                    " CASE dbr.log_send_rate" &_
                        " WHEN 0 THEN -1" &_
                        " ELSE" &_
                            " CAST(dbr.log_send_queue_size AS FLOAT)" &_
                            " / dbr.log_send_rate" &_
                        " END ," &_
                    " -1" &_
                    " ) AS [SynchronizationPerformance] ," &_
                " dbr.synchronization_state_desc AS [SynchronizationState]" &_
        " FROM master.sys.availability_groups AS AG" &_
                " INNER JOIN master.sys.availability_replicas AS AR ON AR.group_id = AG.group_id" &_
                " INNER JOIN master.sys.dm_hadr_database_replica_cluster_states AS dbcs ON dbcs.replica_id = AR.replica_id" &_
                " LEFT OUTER JOIN master.sys.dm_hadr_database_replica_states AS dbr ON dbcs.replica_id = dbr.replica_id" &_
                                                                    " AND dbcs.group_database_id = dbr.group_database_id" &_
                " LEFT OUTER JOIN #tmpdbr_database_replica_states_primary_LCT AS dbrp ON dbr.database_id = dbrp.database_id" &_
                " INNER JOIN master.sys.dm_hadr_availability_replica_states AS arstates ON arstates.replica_id = AR.replica_id" &_
        " WHERE arstates.role_desc = 'SECONDARY'" &_
        " ORDER BY [AvailabilityReplicaServerName]," &_
                " [AvailabilityDatabaseName];", CONN

        Do While NOT RS.EoF
            addOutput("MSSQL_" & instance_id & " " & Replace(RS("AvailabilityDatabaseName"), " ", "_") & " " & Replace(RS("AvailabilityGroupName"), " ", "_") & " " &_
                Replace(RS("EstimatedDataLoss"), " ", "_") & " " & RS("LogSendQueueSize") & " " & RS("LogSendRate") & " " & RS("RedoQueueSize") & " " & RS("RedoRate"))
            RS.MoveNext
        Loop
        RS.Close
        RS.Open "DROP TABLE #tmpdbr_database_replica_states_primary_LCT;", CONN
    ...
    Else
        Do While NOT RS.EoF
            If NOT isnull(RS("backup_finish_date")) and NOT isnull(RS("name")) and NOT isnull(RS("BackupSetName"))then
                addOutput("-" & RS("name") & "-" & RS("BackupSetName") & "-")
    ...
    RS.Open "SELECT name, sjh.run_status, sjh.run_date, sjh.run_time" &_
            ...
            " WHERE CONVERT(VARCHAR(8), sjh.run_date) > GETDATE() - 2" &_
