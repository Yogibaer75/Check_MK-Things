...
For Each instId In instIds.Keys
    ...
    cfg_file = cfg_dir & "\mssql_ha_" & instId & ".ini"
    If Not FSO.FileExists(cfg_file) Then
        cfg_file = cfg_dir & "\mssql_ha.ini"
        If Not FSO.FileExists(cfg_file) Then
            cfg_file = ""
        End If
    End If
    ...
    RS.Open " SELECT DISTINCT" &_
	" dbcs.database_name AS [DatabaseName]" &_
	" FROM master.sys.availability_groups AS AG" &_
	" LEFT OUTER JOIN master.sys.dm_hadr_availability_group_states as agstates" &_
	" ON AG.group_id = agstates.group_id" &_
        " INNER JOIN master.sys.availability_replicas AS AR" &_
        " ON AG.group_id = AR.group_id" &_
        " INNER JOIN master.sys.dm_hadr_availability_replica_states AS arstates" &_
        " ON AR.replica_id = arstates.replica_id AND arstates.is_local = 1" &_
        " INNER JOIN master.sys.dm_hadr_database_replica_cluster_states AS dbcs" &_
        " ON arstates.replica_id = dbcs.replica_id" &_
        " LEFT OUTER JOIN master.sys.dm_hadr_database_replica_states AS dbrs" &_
        " ON dbcs.replica_id = dbrs.replica_id AND dbcs.group_database_id = dbrs.group_database_id" &_
        " WHERE ISNULL(arstates.role, 3) = 2 AND ISNULL(dbcs.is_database_joined, 0) = 1" &_
        " ORDER BY  dbcs.database_name", CONN
    Dim secondaryName, secondaryNames
    Set secondaryNames = CreateObject("Scripting.Dictionary")
    Do While NOT RS.EoF
        secondaryName = RS("DatabaseName")
    ...
    For Each dbName in dbNames.Keys
        If secondaryNames.Exists(dbName) or dbName = "master" or dbName = "model" or dbName = "msdb" or dbName = "USAGE" Then
