...
def discovery_hyperv_vm_nic(section: Section) -> DiscoveryResult:
    for key, values in section.items():
        if "nic.connectionstate" in values:
            yield Service(item=key, parameters={"state": values["nic.connectionstate"]})
...
def _check_connection_state(data: Mapping[str, str], params: NicParams, item: str) -> CheckResult:
    if "connection_state" not in params and params.get("state") is None:
        actual_state = data.get("nic.connectionstate", "unknown")
        yield Result(state=State.OK, summary=f"Connected: {actual_state}")
        return

    if "connection_state" not in params and params.get("state") is not None:
        discovered_state = params["state"]
        actual_state = data.get("nic.connectionstate", "unknown")
        ...
        if actual_state.lower() == discovered_state:
            state = State.OK
        else:
            state = State.WARN
