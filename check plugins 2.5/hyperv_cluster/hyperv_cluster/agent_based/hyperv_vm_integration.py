...
from cmk.plugins.hyperv_cluster.lib import hyperv_vm_convert, parse_hyperv_json_multi
...
SectionJSON = dict[str, dict[str, str | int]]
...
def discovery_hyperv_vm_integration_json(section: Section) -> DiscoveryResult:
    if section:
        yield Service()
...
def process_service_status_json(
    section: SectionJSON,
    match_services: dict[str, ServiceSettings],
    global_default_status: str,
    global_state_if_not_default: int,
    ...
    for key in section:
        ...
        raw_service_status = section[key].get("enabled", False)
        if isinstance(raw_service_status, bool):
            service_status = "active" if raw_service_status else "inactive"
        else:
            service_status = str(raw_service_status)
...
def check_hyperv_vm_integration_json(params: IntegrationServicesParams, section: SectionJSON) -> CheckResult:
    global_default_status = str(params["default_status"])
    global_state_if_not_default = int(params["state_if_not_default"])
    ...
    yield from process_service_status_json(
        section, match_services, global_default_status, global_state_if_not_default
    )
...
agent_section_hyperv_vm_integration_json: AgentSection = AgentSection(
    name="hyperv_vm_integration_json",
    parse_function=parse_hyperv_json_multi,
)
...
check_plugin_hyperv_vm_integration_json = CheckPlugin(
    name="hyperv_vm_integration_json",
    service_name="Hyper-V VM integration services",
    sections=["hyperv_vm_integration_json"],
    discovery_function=discovery_hyperv_vm_integration_json,
    check_function=check_hyperv_vm_integration_json,
    check_default_parameters=hyperv_vm_integration_default_levels,
    check_ruleset_name="hyperv_vm_integration",
)
